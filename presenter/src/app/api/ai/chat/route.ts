...
export async function POST(req: NextRequest) {
  ...
  function invalidPathError(slideIndex: number | undefined, dotPath: string): string | null {
    ...
    if (!pathExistsInShape(shape, dotPath)) {
      ...
      const dragKeys = DRAG_KEYS_BY_TEMPLATE[targetTemplate as keyof typeof DRAG_KEYS_BY_TEMPLATE] ?? [];
      if ((dragKeys as readonly string[]).includes(firstSegment)) {
        const imageFields = Object.keys(shape as Record<string, unknown>).filter((k) => /url/i.test(k));
        const hint = imageFields.length
          ? `it's a layout-only dragKey, not a data field — the actual image field(s) on this template are: ${imageFields.join(', ')}`
          : `it's a layout-only dragKey, not a data field — re-check the slide's JSON data for the real field name`;
        return `error: "${firstSegment}" does not exist as a data path on template "${targetTemplate}" — ${hint}`;
      }
