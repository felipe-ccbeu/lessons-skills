...
import { Fluency1Data, Fluency1Question, StyleOverrides, TextStyleOverride } from '@/lib/types';
  ...
  return (
    <div style={{ position: 'relative', width: 1280, height: 720, background: '#fff', overflow: 'hidden' }}>
      <SlideStagger disabled={editMode}>
        ...
        <div style={{ position: 'absolute', left: 80, top: 290, width: 520 }}>{renderColumn(leftQuestions, 0)}</div>
        <div style={{ position: 'absolute', left: 680, top: 290, width: 520 }}>{renderColumn(rightQuestions, splitAt)}</div>
      </SlideStagger>
      {editMode && (
        <button
          type="button"
